...
const handlerAddParticipant = async (req, res) => {
    ...
    const sql_participant = `INSERT INTO tbl_participants (event_id, name, contingent, category, gender, country) VALUES (?, ?, ?, ?, ?, ?)`;
    for (let i = 0; i < data?.length || 0; i++) {
        db.run(sql_participant, [event_id, data[i].name, data[i].contingent, data[i].category, data[i].gender, data[i].country || ""], function (err) {
            if (err) {
                console.log(err);
