                    ...
                    <div class="flex flex-col justify-center items-center py-4 px-6 bg-blue-600 rounded-md relative text-white">
                        ${STATES.stageKata2.is_final ? `
                                <div class="absolute bottom-12 right-4 flex items-center justify-center">
                                    <span class="text-xl">Perform</span>
                                    <input id="p-2" type="checkbox" ${STATES.stageKata2.active_player == 1 ? 'checked' : ''} onclick="handlerTogglePerform(1)" class="w-5 h-5 ml-2 shadowed">
                                </div>
                            ` : ''
                        }
                        ${STATES.stageKata2.participants[0] != null ? `
...
# We replace from line 386 to the end of the blue grid inputs.
# The blue grid input ends with `</div>\n                ` : ''\n        }\n        </div>`

# So we can search for the start: `    document.getElementById("match-content").innerHTML = \``
# and end at `        </div>\n    \n        <div class="flex flex-col justify-center items-center bg-gray-300`

start_idx = content.find('    document.getElementById("match-content").innerHTML = `\n        <div class="flex flex-col text-white flex-1 min-h-0">')
