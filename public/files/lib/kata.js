...
const handlerSetKataName = () => {
    if (STATES.stageKata != null) {
        STATES.stageKata.kata_name = document.getElementById("kata-name").value;
    ...
    if (type == 'play' && STATES.stageKata.time != 0) {
        ...
        if (STATES.stageKata.play) {
            timeId = setInterval(() => {
                ...
                document.getElementById("timer").textContent = `${Math.floor(STATES.stageKata.time / 60)}:${`0${Math.floor(STATES.stageKata.time % 60)}`.slice(-2)}.${Math.floor((STATES.stageKata.time % 1) * 10)}`
                if (STATES.stageKata.time == 0) STATES.stageKata.play = false
                if (STATES.stageKata.time <= 16.10 && !STATES.shortBellPlayed) {
                    STATES.shortBellPlayed = true;
                    if (document.getElementById('short-bell')) document.getElementById('short-bell').play();
                }
                if (STATES.stageKata.time > 16.10) STATES.shortBellPlayed = false;
                STATES.stageKata.id = Math.floor(Math.random() * 1000000)
...
const handlerSaveMatch = () => {
    ...
    if (STATES.stageKata.participants[0] != null) {
        let scores1 = [STATES.stageKata.participants[0].g1, STATES.stageKata.participants[0].g2, STATES.stageKata.participants[0].g3, STATES.stageKata.participants[0].g4, STATES.stageKata.participants[0].g5].map(v => parseFloat(v) || 0).sort((a, b) => a - b);
        let point1 = scores1[1] + scores1[2] + scores1[3];
        grade1 = `${STATES.stageKata.participants[0]?.g1}, ${STATES.stageKata.participants[0]?.g2}, ${STATES.stageKata.participants[0]?.g3}, ${STATES.stageKata.participants[0]?.g4}, ${STATES.stageKata.participants[0]?.g5}, Point ${point1.toFixed(2)}`;
    }

    let grade2 = '';
    if (STATES.stageKata.participants[1] != null) {
        let scores2 = [STATES.stageKata.participants[1].g1, STATES.stageKata.participants[1].g2, STATES.stageKata.participants[1].g3, STATES.stageKata.participants[1].g4, STATES.stageKata.participants[1].g5].map(v => parseFloat(v) || 0).sort((a, b) => a - b);
        let point2 = scores2[1] + scores2[2] + scores2[3];
        grade2 = `${STATES.stageKata.participants[1]?.g1}, ${STATES.stageKata.participants[1]?.g2}, ${STATES.stageKata.participants[1]?.g3}, ${STATES.stageKata.participants[1]?.g4}, ${STATES.stageKata.participants[1]?.g5}, Point ${point2.toFixed(2)}`;
    }
    ...
    document.getElementById("match-content").innerHTML = `
        <div class="flex text-white flex-1 min-h-0">
            ${STATES.stageKata.participants[1] != null ? `
                    <div class="flex flex-1 flex-col justify-center items-center p-6 h-full bg-blue-600 rounded-md relative">
                        ...
                        <h1 class="text-[7em] font-extrabold mt-2 leading-none">${STATES.stageKata?.participants[1]?.point}</h1>
                    </div>
                ` : ''
        }
            ...
            ${STATES.stageKata.participants[0] != null ? `
                    <div class="flex flex-1 flex-col justify-center items-center p-6 h-full bg-red-600 rounded-md relative">
                        ...
                        <h1 class="text-[7em] font-extrabold mt-2 leading-none">${STATES.stageKata?.participants[0]?.point}</h1>
                    </div>
                ` : ''
        }
        </div>
    
        <div class="grid ${(STATES.stageKata.participants[0] != null && STATES.stageKata.participants[1] != null) ? 'grid-cols-2' : 'grid-cols-1'} gap-3 my-2">
            ${STATES.stageKata.participants[1] != null ? `
                    <div class="flex justify-between items-center flex-row flex-nowrap gap-1 bg-blue-100 p-1 rounded-lg w-full">
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKata.participants[1]?.c1 ? 'checked' : ''} onclick="handlerToggleField(1, 1)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-1-1" type="text" value="${STATES.stageKata.participants[1]?.g1 != 0 ? STATES.stageKata.participants[1]?.g1 : ''}" onchange="handlerSetGrade(1, 1)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J1" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKata.participants[1]?.c2 ? 'checked' : ''} onclick="handlerToggleField(1, 2)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-1-2" type="text" value="${STATES.stageKata.participants[1]?.g2 != 0 ? STATES.stageKata.participants[1]?.g2 : ''}" onchange="handlerSetGrade(1, 2)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J2" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKata.participants[1]?.c3 ? 'checked' : ''} onclick="handlerToggleField(1, 3)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-1-3" type="text" value="${STATES.stageKata.participants[1]?.g3 != 0 ? STATES.stageKata.participants[1]?.g3 : ''}" onchange="handlerSetGrade(1, 3)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J3" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKata.participants[1]?.c4 ? 'checked' : ''} onclick="handlerToggleField(1, 4)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-1-4" type="text" value="${STATES.stageKata.participants[1]?.g4 != 0 ? STATES.stageKata.participants[1]?.g4 : ''}" onchange="handlerSetGrade(1, 4)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J4" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKata.participants[1]?.c5 ? 'checked' : ''} onclick="handlerToggleField(1, 5)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-1-5" type="text" value="${STATES.stageKata.participants[1]?.g5 != 0 ? STATES.stageKata.participants[1]?.g5 : ''}" onchange="handlerSetGrade(1, 5)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J5" />
                        </div>
                    </div>
                ` : ''
        }
            ${STATES.stageKata.participants[0] != null ? `
                    <div class="flex justify-between items-center flex-row flex-nowrap gap-1 bg-red-100 p-1 rounded-lg w-full">
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKata.participants[0]?.c1 ? 'checked' : ''} onclick="handlerToggleField(0, 1)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-0-1" type="text" value="${STATES.stageKata.participants[0]?.g1 != 0 ? STATES.stageKata.participants[0]?.g1 : ''}" onchange="handlerSetGrade(0, 1)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J1" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKata.participants[0]?.c2 ? 'checked' : ''} onclick="handlerToggleField(0, 2)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-0-2" type="text" value="${STATES.stageKata.participants[0]?.g2 != 0 ? STATES.stageKata.participants[0]?.g2 : ''}" onchange="handlerSetGrade(0, 2)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J2" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKata.participants[0]?.c3 ? 'checked' : ''} onclick="handlerToggleField(0, 3)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-0-3" type="text" value="${STATES.stageKata.participants[0]?.g3 != 0 ? STATES.stageKata.participants[0]?.g3 : ''}" onchange="handlerSetGrade(0, 3)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J3" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKata.participants[0]?.c4 ? 'checked' : ''} onclick="handlerToggleField(0, 4)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-0-4" type="text" value="${STATES.stageKata.participants[0]?.g4 != 0 ? STATES.stageKata.participants[0]?.g4 : ''}" onchange="handlerSetGrade(0, 4)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J4" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKata.participants[0]?.c5 ? 'checked' : ''} onclick="handlerToggleField(0, 5)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-0-5" type="text" value="${STATES.stageKata.participants[0]?.g5 != 0 ? STATES.stageKata.participants[0]?.g5 : ''}" onchange="handlerSetGrade(0, 5)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J5" />
                        </div>
                    </div>                
                ` : ''
        }
        ...
        <div class="flex flex-col justify-center items-center bg-gray-300 p-2 rounded-lg">
            <div class="flex gap-4 mb-2">
                ...
                <div id="timer" class="text-6xl font-bold mx-4 text-center" style="width: 280px; flex-shrink: 0;">${Math.floor(STATES.stageKata.time / 60)}:${`0${Math.floor(STATES.stageKata.time % 60)}`.slice(-2)}.${Math.floor((STATES.stageKata.time % 1) * 10)}</div>
                <button class="py-2 px-4 bg-gray-600 text-white rounded-lg hover:bg-gray-500" onclick="handlerMatchControl(0, '+1s')">+1s</button>
                <button class="py-2 px-4 bg-gray-600 text-white rounded-lg hover:bg-gray-500" onclick="handlerMatchControl(0, '+30s')">+30s</button>
                <button class="py-2 px-4 bg-gray-600 text-white rounded-lg hover:bg-gray-500" onclick="handlerMatchControl(0, '+60s')">+60s</button>
            </div>
            ...
            <button class="py-2 px-6 ${STATES.stageKata.play ? "bg-red-600 text-white" : "bg-green-600 text-white"} text-white rounded-lg ${STATES.stageKata.time > 0 ? STATES.stageKata.play ? "hover:bg-red-500" : "hover:bg-green-500" : 'opacity-50'} flex items-center mt-2" onclick="handlerMatchControl(0, 'play')">
...
const handlerOnload = async () => {
    ...
    socket.on("update-grade", (data) => {
        ...
        const isArenaMatch = STATES.stageKata != null && String(STATES.stageKata?.tatami).replace(/[^0-9]/g, '') === String(data?.jury?.arenaId).replace(/[^0-9]/g, '');
        console.log("Validation: ", { isEventMatch, isArenaMatch, stageKata: STATES.stageKata });

        if (isEventMatch && STATES.stageKata != null && isArenaMatch) {
            if (STATES.stageKata.is_locked || STATES.stageKata.participants[data?.participant]?.is_locked) return;
            ...
            STATES.stageKata.id = Math.floor(Math.random() * 1000)
            STATES.stageKata.participants[data?.participant][`g${juryId}`] = parseFloat(data?.grade).toFixed(1)
            localStorage.setItem("stage_kata", JSON.stringify(STATES.stageKata))
            handlerSetPoint()
        }
    })
    ...
    if (stageKata && parsedStageKumite?.event_id == STATES.event?.id) {
        ...
        document.getElementById("match-handler").classList.remove("hidden");
        if (document.getElementById("kata-name")) {
            document.getElementById("kata-name").value = STATES.stageKata.kata_name || "";
        }
