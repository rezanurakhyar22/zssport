    ...
    for (let i = 0; i < brackets.length; i++) {
        for (let j = 0; j < brackets[i].length; j++) {
            for (let k = 0; k < brackets[i][j].length; k++) {
                if (brackets[i][j][k]?.participant?.includes(null) && brackets[i][j][k].coordinate) {
    ...
    if (totalParticipants === 2) {
        return [
            [
                [
                    {
                        participant: [shuffledParticipants[0], shuffledParticipants[1]],
                        coordinate: ''
                    }
                ]
            ]
...
const scaleBracket = () => {
    ...
    // Target skala 80% (0.8) dari layar, jika melebihi layar maka diturunkan agar tidak ada scrollbar
    const scaleX = (contentWidth * 0.8) / wrapperWidth;
    const scaleY = (contentHeight * 0.8) / wrapperHeight;
...
const renderBracket = () => {
                        ...
                        if (matches?.length > 0) {
                            for (let k = 0; k < matches.length; k++) {
                                ...
                                if (matches[k] == null || Object.keys(matches[k]).length == 0) {
                                    row1 += `
                                        <div class="w-[290px] my-1 mx-[1.5em] overflow-hidden opacity-0">
                                            <div class="flex">
                                                <div class="flex flex-col flex-1 p-2">
                                                    <span class="text-center text-base font-bold line-clamp-2">-</span>
                                                    <span class="text-center text-xs font-bold">-</span>
                                                </div>
                                            </div>
                                        </div>
                                    `
                                    ...
                                    row1 += `<div id='${i}-${j}-${k}' class="w-[290px] text-white my-1 mx-[1.5em] overflow-hidden rounded-md ">`
                                        ...
                                        if (participant == null || Object.keys(participant).length == 0) {
                                            row1 += `
                                                <div id="${i}-${j}-${k}-${l}" class="flex bg-gradient-to-r from-[${l == 0 ? '#eb3434' : '#3437eb'}] to-gray-600 ${l == 0 ? 'mb-[.18em]' : ''}" allowdrop="true" ondrop="handlerDrop(event)" ondragover="handlerAllowDrop(event)">
                                                    <div class="flex flex-col flex-1 p-2">
                                                        <span id="name-${i}-${j}-${k}-${l}" class="text-center text-base font-bold line-clamp-2">${participant == null ? 'BYE' : '-'}</span>
                                                        <span id="contingent-${i}-${j}-${k}-${l}" class="text-center text-xs font-bold">-</span>
                                                    </div>
                                            ...
                                            row1 += `
                                                <div id="${i}-${j}-${k}-${l}" class="flex bg-gradient-to-r from-[${l == 0 ? '#eb3434' : '#3437eb'}] to-gray-600 ${l == 0 ? 'mb-[.18em]' : ''} hover:cursor-pointer hover:bg-gray-600" draggable="true" allowdrop="true" ondragover="handlerAllowDrop(event)" ondragstart="handlerDragStart(event)" ondragend="handlerDragEnd(event)" ondrop="font-bold" ondrop="handlerDrop(event)" ondblclick="handlerClear(event)">
                                                    <div class="flex flex-col flex-1 p-2">
                                                        <span id="name-${i}-${j}-${k}-${l}" class="text-center text-base font-bold line-clamp-2">${participant.name}</span>
                                                        <span id="contingent-${i}-${j}-${k}-${l}" class="text-center text-xs font-bold">(${participant.contingent?.toUpperCase()})</span>
                                                    </div>
                                                    ${l == 0 ? '<div class="w-[.5em] bg-[#eb3434]"></div>'
                                                    : '<div class="w-[.5em] bg-[#3437eb]"></div>'
                                                }
                            ...
                            row1 += `<div class="w-[290px] my-4"></div>`
                        }
                        row1 += `</div>`
                    }
                }
                    ...
                    if (j == 1) {
                        ...
                        if (matches?.length > 0) {
                            for (let k = 0; k < matches.length; k++) {
                                ...
                                if (matches[k] == null || Object.keys(matches[k]).length == 0) {
                                    row2 += `
                                        <div class="w-[290px] text-white my-1 mx-[1.5em] overflow-hidden rounded-md hidden">
                                            <div class="flex">
                                                <div class="flex flex-col flex-1 p-2">
                                                    <span class="text-center text-base font-bold line-clamp-2">-</span>
                                                    <span class="text-center text-xs font-bold">-</span>
                                    ...
                                    const groups = matches[k]?.participant || []
                                    row2 += `<div id='${i}-${j}-${k}' class="w-[290px] text-white my-1 mx-[1.5em] overflow-hidden rounded-md ">`
                                    for (let l = 0; l < groups?.length; l++) {
                                        const participant = groups[l]
                                        if (participant == null || Object.keys(participant).length == 0) {
                                            row2 += `
                            ...
                            row2 += `<div class="w-[290px] my-4"></div>`
                        }
                        row2 += `</div>`
                    }
                }
    ...
    content.innerHTML = `
        ...
        <div id="bracket-wrapper" class="flex flex-row justify-center items-center origin-top transition-all duration-150" style="flex-shrink: 0; transform-origin: top center;">
            <div class="flex flex-col" style="flex-shrink: 0;">
                ${row1}
                ${row2}
            </div>
            <div class="flex flex-col justify-center items-center" style="flex-shrink: 0;">
                <div id='${lastRoundIndex}-0-0' class="w-[290px] text-white mx-[1.5em] overflow-hidden rounded-md " style="flex-shrink: 0;">
                    <div id="${lastRoundIndex}-0-0-0" class="flex bg-gradient-to-r from-[#eb3434] to-gray-600 mb-[.18em] hover:cursor-pointer hover:bg-gray-600" draggable="true" allowdrop="true" ondragover="handlerAllowDrop(event)" ondragstart="handlerDragStart(event)" ondragend="handlerDragEnd(event)" ondrop="handlerDrop(event)" ondblclick="handlerClear(event)">
                        <div class="flex flex-col flex-1 p-2">
                            <span id="name-${lastRoundIndex}-0-0-0" class="text-center text-base font-bold line-clamp-2">${lastRoundData?.[0]?.name || '-'}</span>
                            <span id="contingent-${lastRoundIndex}-0-0-0" class="text-center text-xs font-bold">${lastRoundData?.[0]?.contingent ? `(${lastRoundData?.[0]?.contingent?.toUpperCase()})` : '-'}</span>
                        </div>
                    ...
                    <div id="${lastRoundIndex}-0-0-1" class="flex bg-gradient-to-r from-[#3437eb] to-gray-600 hover:cursor-pointer hover:bg-gray-600" draggable="true" allowdrop="true" ondragover="handlerAllowDrop(event)" ondragstart="handlerDragStart(event)" ondragend="handlerDragEnd(event)" ondrop="handlerDrop(event)" ondblclick="handlerClear(event)">
                        <div class="flex flex-col flex-1 p-2">
                            <span id="name-${lastRoundIndex}-0-0-1" class="text-center text-base font-bold line-clamp-2">${lastRoundData?.[1]?.name || '-'}</span>
                            <span id="contingent-${lastRoundIndex}-0-0-1" class="text-center text-xs font-bold">${lastRoundData?.[1]?.contingent ? `(${lastRoundData?.[1]?.contingent?.toUpperCase()})` : '-'}</span>
                        </div>
                        <div class="w-[.5em] bg-[#3437eb]"></div>
                    </div>
                </div>
            </div>
            <div id="winner" class="flex flex-col justify-center items-center" style="flex-shrink: 0;">
                <div class="w-[290px] text-white my-4 mx-[2em] overflow-hidden rounded-md hover:cursor-pointer hover:bg-gray-600" allowdrop="true" ondragover="handlerAllowDrop(event)" ondrop="handlerDropWinner(event)" ondblclick="handlerClearWinner(event)">
                    <div class="flex bg-gray-500 relative">
                        <div class="flex flex-col flex-1 p-2">
                            <span class="text-center text-lg font-bold line-clamp-2">${STATES.winners?.[0]?.name?.toUpperCase() || '-'}</span>
                            <span class="text-center text-xs font-bold">${STATES.winners?.[0]?.contingent ? `(${STATES.winners?.[0]?.contingent?.toUpperCase()})` : '-'}</span>
                        </div>
...
const handlerOnload = async () => {
    ...
    if (contentEl) {
        ...
        if (contentEl.parentElement) {
            contentEl.parentElement.style.overflow = "hidden";
        }
    }
