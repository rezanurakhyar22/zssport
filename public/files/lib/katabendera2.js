...
const handlerGetAllMatches = async () => {
    ...
    if (data?.statusCode == 200) {
        const filtered = data.data?.filter(d => d?.match_type == "KATA BENDERA 2" || d?.match_type == "KATA BENDERA 2 REGU")
...
const handlerCloseModal = (add) => {
    ...
    if (add) {
        ...
        if (participant1.value?.split('|')?.[0]) {
            stageKataBendera2.participants.push({
        ...
        } else stageKataBendera2.participants.push(null)
        ...
        localStorage.setItem("stage_kata_bendera2", JSON.stringify(stageKataBendera2))
        STATES.stageKataBendera2 = stageKataBendera2
        document.getElementById("match-handler").classList.remove("hidden");
        document.getElementById("title").innerText = `${stageKataBendera2?.match_id} | ${stageKataBendera2?.tatami} | ${stageKataBendera2?.category?.toUpperCase()}`
...
const handlerDeleteStageMatch = () => {
    ...
    if (conf) {
        localStorage.removeItem("stage_kata_bendera2")
        STATES.stageKataBendera2 = null
        document.getElementById("match-handler").classList.add("hidden");
        document.getElementById("title").innerText = `KATA BENDERA 2 | EVENT ${STATES.event?.name?.toUpperCase()}`
...
const handlerMatchControl = (i, type) => {
    ...
    if (type == 'play' && STATES.stageKataBendera2.time != 0) {
        STATES.stageKataBendera2.play = !STATES.stageKataBendera2?.play

        if (STATES.stageKataBendera2.play) {
            timeId = setInterval(() => {
                STATES.stageKataBendera2.time = Math.max(STATES.stageKataBendera2.time - .01, 0)
                document.getElementById("timer").textContent = `${Math.floor(STATES.stageKataBendera2.time / 60)}:${`0${Math.floor(STATES.stageKataBendera2.time % 60)}`.slice(-2)}.${Math.floor((STATES.stageKataBendera2.time % 1) * 10)}`
                if (STATES.stageKataBendera2.time == 0) STATES.stageKataBendera2.play = false
                if (STATES.stageKataBendera2.time <= 16.10 && !STATES.shortBellPlayed) {
                    STATES.shortBellPlayed = true;
                    if (document.getElementById('short-bell')) document.getElementById('short-bell').play();
                }
                if (STATES.stageKataBendera2.time > 16.10) STATES.shortBellPlayed = false;
                STATES.stageKataBendera2.id = Math.floor(Math.random() * 1000000)
                STATES.stageKataBendera2.rest = false
                localStorage.setItem("stage_kata_bendera2", JSON.stringify(STATES.stageKataBendera2))
                if (STATES.stageKataBendera2.time == 0) {
                    clearInterval(timeId)
                    timeId = null
                    handlerRenderMatch()
                    if (document.getElementById('long-bell')) document.getElementById('long-bell').play()
                }
            }, 10);
        } else {
            clearInterval(timeId)
        ...
        STATES.stageKataBendera2.rest = true
    }

    if (type == 'set') {
        STATES.stageKataBendera2.time = document.getElementById("time").value
        document.getElementById("time").value = 0
    }
    if (type == '-1s') STATES.stageKataBendera2.time = Math.max(STATES.stageKataBendera2.time - 1, 0)
    if (type == '-30s') STATES.stageKataBendera2.time = Math.max(STATES.stageKataBendera2.time - 30, 0)
    if (type == '-60s') STATES.stageKataBendera2.time = Math.max(STATES.stageKataBendera2.time - 60, 0)
    if (type == '+1s') STATES.stageKataBendera2.time = STATES.stageKataBendera2.time + 1
    if (type == '+30s') STATES.stageKataBendera2.time = STATES.stageKataBendera2.time + 30
    if (type == '+60s') STATES.stageKataBendera2.time = STATES.stageKataBendera2.time + 60
    if (type == 'reset') {
        STATES.stageKataBendera2.time = 0
        STATES.stageKataBendera2.play = false
        clearInterval(timeId)
        timeId = null
    }
...
const handlerCheckWinner = () => {
    ...
    if (STATES.stageKataBendera2.participants[0].point > STATES.stageKataBendera2.participants[1].point) winner = STATES.stageKataBendera2.participants[0].id
    if (STATES.stageKataBendera2.participants[1].point > STATES.stageKataBendera2.participants[0].point) winner = STATES.stageKataBendera2.participants[1].id
...
const handlerSaveMatch = () => {
    ...
    const isRegu = !(STATES.stageKataBendera2.participants[0] != null && STATES.stageKataBendera2.participants[1] != null)

    let winner = isRegu ? '' : document.getElementById("w-1").checked ? STATES.stageKataBendera2.participants[0].id : document.getElementById("w-2").checked ? STATES.stageKataBendera2.participants[1].id : handlerCheckWinner()
    let grade1 = STATES.stageKataBendera2.participants[0] == null ? '' : `${STATES.stageKataBendera2.participants[0]?.g1}, ${STATES.stageKataBendera2.participants[0]?.g2}, ${STATES.stageKataBendera2.participants[0]?.g3}, ${STATES.stageKataBendera2.participants[0]?.g4}, ${STATES.stageKataBendera2.participants[0]?.g5}, Point ${STATES.stageKataBendera2.participants[0]?.point}`
    let grade2 = STATES.stageKataBendera2.participants[1] == null ? '' : `${STATES.stageKataBendera2.participants[1]?.g1}, ${STATES.stageKataBendera2.participants[1]?.g2}, ${STATES.stageKataBendera2.participants[1]?.g3}, ${STATES.stageKataBendera2.participants[1]?.g4}, ${STATES.stageKataBendera2.participants[1]?.g5}, Point ${STATES.stageKataBendera2.participants[1]?.point}`

    const data = {
        event_id: STATES.event.id,
        category: STATES.stageKataBendera2.category,
        match_type: isRegu ? "KATA BENDERA 2 REGU" : "KATA BENDERA 2",
        winner_id: winner,
        arena: STATES.stageKataBendera2.tatami,
        time: "",
        participants: []
    }

    if (STATES.stageKataBendera2.participants[0] != null) {
        data.participants.push({
            id: STATES.stageKataBendera2.participants[0].id,
            grade: grade1
        })
    }

    if (STATES.stageKataBendera2.participants[1] != null) {
        data.participants.push({
            id: STATES.stageKataBendera2.participants[1].id,
            grade: grade2
        })
    }
    ...
    fetch(`/api/match/create`, {
        ...
        .then(res => {
            if (res?.statusCode == 200) {
                STATES.stageKataBendera2 = null
                localStorage.removeItem("stage_kata_bendera2")
                localStorage.removeItem("confetti_kata_bendera2")
...
const handlerToggleCheckbox = (n) => {
    ...
    if (document.getElementById(`w-1`).checked || document.getElementById(`w-2`).checked) localStorage.setItem("confetti_kata_bendera2", `win-${n == 1 ? 'red' : 'blue'}-${Math.floor(Math.random() * 1000)}`)
}

const handlerTogglePerform = (n) => {

    STATES.stageKataBendera2.active_player = n
    localStorage.setItem("stage_kata_bendera2", JSON.stringify(STATES.stageKataBendera2))
    handlerRenderMatch()
}
...
const handlerSetPoint = () => {
    ...
    STATES.stageKataBendera2?.participants?.forEach((item, i) => {
        ...
        if (item != null) {
            ...
            if (grades?.length == 3 && allSet) STATES.stageKataBendera2.participants[i].point = parseFloat(grades.reduce((a, b) => a + b)).toFixed(2)
            else STATES.stageKataBendera2.participants[i].point = 0
        }
    })
...
const handlerToggleField = (index, jury) => {
    ...
    STATES.stageKataBendera2.id = Math.floor(Math.random() * 1000)
    STATES.stageKataBendera2.participants[index][`c${jury}`] = !STATES.stageKataBendera2.participants[index][`c${jury}`]
    localStorage.setItem('stage_kata_bendera2', JSON.stringify(STATES.stageKataBendera2))
...
const handlerSetGrade = (index, jury) => {
    ...
    STATES.stageKataBendera2.id = Math.floor(Math.random() * 1000)
    STATES.stageKataBendera2.participants[index][`g${jury}`] = value
    localStorage.setItem('stage_kata_bendera2', JSON.stringify(STATES.stageKataBendera2))
...
const handlerRenderMatch = () => {
    ...
    document.getElementById("match-content").innerHTML = `
        <div class="flex text-white flex-1 min-h-0">
            ${STATES.stageKataBendera2.participants[1] != null ? `
                    <div class="flex flex-1 flex-col justify-center items-center p-6 h-full bg-blue-600 rounded-md relative">
                        ${STATES.stageKataBendera2.is_final ? `
                                <div class="absolute bottom-12 right-4 flex items-center justify-center">
                                    <span class="text-xl">Perform</span>
                                    <input id="p-2" type="checkbox" ${STATES.stageKataBendera2.active_player == 1 ? 'checked' : ''} onclick="handlerTogglePerform(1)" class="w-5 h-5 ml-2 shadowed">
                                </div>
                            ` : ''
            }
                        ${STATES.stageKataBendera2.participants[0] != null ? `
                        ...
                        <h1 class="text-4xl font-bold line-clamp-1">${STATES.stageKataBendera2?.participants[1]?.name}</h1>
                        <h2 class="text-2xl font-bold">(${STATES.stageKataBendera2?.participants[1]?.contingent})</h2>
                        <h1 class="text-[7em] font-extrabold mt-2 leading-none">${STATES.stageKataBendera2?.participants[1]?.point}</h1>
                    </div>
                ` : ''
        }
            ${STATES.stageKataBendera2.participants[0] == null || STATES.stageKataBendera2.participants[1] == null ? '' : '<div class="mx-2"></div>'}
            ${STATES.stageKataBendera2.participants[0] != null ? `
                    <div class="flex flex-1 flex-col justify-center items-center p-6 h-full bg-red-600 rounded-md relative">
                        ${STATES.stageKataBendera2.is_final ? `
                                <div class="absolute bottom-12 left-4 flex items-center justify-center">
                                    <input id="p-1" type="checkbox" ${STATES.stageKataBendera2.active_player == 0 ? 'checked' : ''} onclick="handlerTogglePerform(0)" class="w-5 h-5 mr-2 shadowed">
                                    <span class="text-xl">Perform</span>
                                </div>
                            ` : ''
            }
                        ...
                        <h1 class="text-4xl font-bold line-clamp-1">${STATES.stageKataBendera2?.participants[0]?.name}</h1>
                        <h2 class="text-2xl font-bold">(${STATES.stageKataBendera2?.participants[0]?.contingent})</h2>
                        <h1 class="text-[7em] font-extrabold mt-2 leading-none">${STATES.stageKataBendera2?.participants[0]?.point}</h1>
                    </div>
                ` : ''
        }
        </div>
    
        <div class="grid ${(STATES.stageKataBendera2.participants[0] != null && STATES.stageKataBendera2.participants[1] != null) ? 'grid-cols-2' : 'grid-cols-1'} gap-3 my-2">
            ${STATES.stageKataBendera2.participants[1] != null ? `
                    <div class="flex justify-between items-center flex-row flex-nowrap gap-1 bg-blue-100 p-1 rounded-lg w-full">
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKataBendera2.participants[1]?.c1 ? 'checked' : ''} onclick="handlerToggleField(1, 1)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-1-1" type="text" value="${STATES.stageKataBendera2.participants[1]?.g1 != 0 ? STATES.stageKataBendera2.participants[1]?.g1 : ''}" onchange="handlerSetGrade(1, 1)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J1" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKataBendera2.participants[1]?.c2 ? 'checked' : ''} onclick="handlerToggleField(1, 2)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-1-2" type="text" value="${STATES.stageKataBendera2.participants[1]?.g2 != 0 ? STATES.stageKataBendera2.participants[1]?.g2 : ''}" onchange="handlerSetGrade(1, 2)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J2" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKataBendera2.participants[1]?.c3 ? 'checked' : ''} onclick="handlerToggleField(1, 3)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-1-3" type="text" value="${STATES.stageKataBendera2.participants[1]?.g3 != 0 ? STATES.stageKataBendera2.participants[1]?.g3 : ''}" onchange="handlerSetGrade(1, 3)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J3" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKataBendera2.participants[1]?.c4 ? 'checked' : ''} onclick="handlerToggleField(1, 4)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-1-4" type="text" value="${STATES.stageKataBendera2.participants[1]?.g4 != 0 ? STATES.stageKataBendera2.participants[1]?.g4 : ''}" onchange="handlerSetGrade(1, 4)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J4" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKataBendera2.participants[1]?.c5 ? 'checked' : ''} onclick="handlerToggleField(1, 5)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-1-5" type="text" value="${STATES.stageKataBendera2.participants[1]?.g5 != 0 ? STATES.stageKataBendera2.participants[1]?.g5 : ''}" onchange="handlerSetGrade(1, 5)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J5" />
                        </div>
                    </div>
                ` : ''
        }
            ${STATES.stageKataBendera2.participants[0] != null ? `
                    <div class="flex justify-between items-center flex-row flex-nowrap gap-1 bg-red-100 p-1 rounded-lg w-full">
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKataBendera2.participants[0]?.c1 ? 'checked' : ''} onclick="handlerToggleField(0, 1)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-0-1" type="text" value="${STATES.stageKataBendera2.participants[0]?.g1 != 0 ? STATES.stageKataBendera2.participants[0]?.g1 : ''}" onchange="handlerSetGrade(0, 1)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J1" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKataBendera2.participants[0]?.c2 ? 'checked' : ''} onclick="handlerToggleField(0, 2)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-0-2" type="text" value="${STATES.stageKataBendera2.participants[0]?.g2 != 0 ? STATES.stageKataBendera2.participants[0]?.g2 : ''}" onchange="handlerSetGrade(0, 2)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J2" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKataBendera2.participants[0]?.c3 ? 'checked' : ''} onclick="handlerToggleField(0, 3)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-0-3" type="text" value="${STATES.stageKataBendera2.participants[0]?.g3 != 0 ? STATES.stageKataBendera2.participants[0]?.g3 : ''}" onchange="handlerSetGrade(0, 3)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J3" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKataBendera2.participants[0]?.c4 ? 'checked' : ''} onclick="handlerToggleField(0, 4)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-0-4" type="text" value="${STATES.stageKataBendera2.participants[0]?.g4 != 0 ? STATES.stageKataBendera2.participants[0]?.g4 : ''}" onchange="handlerSetGrade(0, 4)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J4" />
                        </div>
                        <div class="flex bg-white shadowed rounded-md overflow-hidden flex-1 min-w-0 items-center justify-between">
                            <input type="checkbox" ${STATES.stageKataBendera2.participants[0]?.c5 ? 'checked' : ''} onclick="handlerToggleField(0, 5)" class="w-4 h-4 ml-2 mr-1 my-2 shrink-0">
                            <input id="g-0-5" type="text" value="${STATES.stageKataBendera2.participants[0]?.g5 != 0 ? STATES.stageKataBendera2.participants[0]?.g5 : ''}" onchange="handlerSetGrade(0, 5)" class="bg-[#f5f5f5] w-full min-w-0 appearance-none border-none outline-none focus:ring-0 rounded-e-md text-gray-900 focus:border-none text-base px-3 py-2 text-center" placeholder="J5" />
                        </div>
                    </div>                
                ` : ''
        }
        ...
        <div class="flex flex-col justify-center items-center bg-gray-300 p-2 rounded-lg">
            <div class="flex gap-4 mb-2">
                ...
                <div id="timer" class="text-6xl font-bold mx-4 text-center" style="width: 280px; flex-shrink: 0;">${Math.floor(STATES.stageKataBendera2.time / 60)}:${`0${Math.floor(STATES.stageKataBendera2.time % 60)}`.slice(-2)}.${Math.floor((STATES.stageKataBendera2.time % 1) * 10)}</div>
                <button class="py-2 px-4 bg-gray-600 text-white rounded-lg hover:bg-gray-500" onclick="handlerMatchControl(0, '+1s')">+1s</button>
                <button class="py-2 px-4 bg-gray-600 text-white rounded-lg hover:bg-gray-500" onclick="handlerMatchControl(0, '+30s')">+30s</button>
                <button class="py-2 px-4 bg-gray-600 text-white rounded-lg hover:bg-gray-500" onclick="handlerMatchControl(0, '+60s')">+60s</button>
            </div>
            ...
            <button class="py-2 px-6 ${STATES.stageKataBendera2.play ? "bg-red-600 text-white" : "bg-green-600 text-white"} text-white rounded-lg ${STATES.stageKataBendera2.time > 0 ? STATES.stageKataBendera2.play ? "hover:bg-red-500" : "hover:bg-green-500" : 'opacity-50'} flex items-center mt-2" onclick="handlerMatchControl(0, 'play')">
                ${STATES.stageKataBendera2.play ? "<i class='bx bx-stop text-5xl'></i>" : "<i class='bx bx-play text-5xl'></i>"
        }
            </button>
        </div>
    `
...
const handlerRenderFormModal = () => {
    ...
    modal.innerHTML = `
        <div class="bg-white p-6 rounded-lg w-[30em]">
            <h2 class="text-[1.5em] font-bold text-gray mb-4">Buat Pertandingan Kata Bendera 2</h2>
...
const handlerRenderTable = () => {
    ...
    if (STATES.stageKataBendera2 != null) return
...
const handlerOnload = async () => {
    ...
    socket.on("update-grade", (data) => {
        ...
        const isArenaMatch = STATES.stageKataBendera2 != null && String(STATES.stageKataBendera2?.tatami).replace(/[^0-9]/g, '') === String(data?.jury?.arenaId).replace(/[^0-9]/g, '');
        console.log("Validation: ", { isEventMatch, isArenaMatch, stageKataBendera2: STATES.stageKataBendera2 });

        if (isEventMatch && STATES.stageKataBendera2 != null && isArenaMatch) {
            if (STATES.stageKataBendera2.is_locked || STATES.stageKataBendera2.participants[data?.participant]?.is_locked) return;
            ...
            STATES.stageKataBendera2.id = Math.floor(Math.random() * 1000)
            STATES.stageKataBendera2.participants[data?.participant][`g${juryId}`] = parseFloat(data?.grade).toFixed(1)
            localStorage.setItem("stage_kata_bendera2", JSON.stringify(STATES.stageKataBendera2))
            handlerSetPoint()
        }
    })

    const stageKataBendera2 = await localStorage.getItem("stage_kata_bendera2")
    const parsedStageKumite = JSON.parse(stageKataBendera2)

    if (stageKataBendera2 && parsedStageKumite?.event_id == STATES.event?.id) {
        STATES.stageKataBendera2 = JSON.parse(stageKataBendera2)
        document.getElementById("match-handler").classList.remove("hidden");
        document.getElementById("title").innerText = `${STATES.stageKataBendera2?.match_id} | ${STATES.stageKataBendera2?.tatami} | ${STATES.stageKataBendera2?.category?.toUpperCase()}`
        handlerRenderMatch()
    } else {
        document.getElementById("match-handler").classList.add("hidden");
        document.getElementById("main-content").classList.remove("hidden");
...
const handlerOnUnload = () => {
    ...
    clearInterval(timeId)
    if (STATES.stageKataBendera2 != null) {
        STATES.stageKataBendera2.play = false
        localStorage.setItem("stage_kata_bendera2", JSON.stringify(STATES.stageKataBendera2))
    }
}
...
const handlerOnKeydown = (e) => {
    ...
    if (e.keyCode == 32 && STATES.stageKataBendera2 != null) {
        e.preventDefault()
...
const handlerSetKataName = () => {
    if (STATES.stageKataBendera2 != null) {
        STATES.stageKataBendera2.kata_name = document.getElementById("kata-name").value;
        STATES.stageKataBendera2.id = Math.floor(Math.random() * 1000000);
        localStorage.setItem("stage_kata_bendera2", JSON.stringify(STATES.stageKataBendera2));
    }
}
...
document.addEventListener('click', (e) => {
    const dropdown = document.getElementById('kata-dropdown');
    const input = document.getElementById('kata-name');
