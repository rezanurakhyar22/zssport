    ...
    if (type == 'play' && STATES.stageKumite.time != 0) {
        ...
        if (STATES.stageKumite.play) {
            timeId = setInterval(() => {
                ...
                document.getElementById("timer").textContent = `${Math.floor(STATES.stageKumite.time / 60)}:${`0${Math.floor(STATES.stageKumite.time%60)}`.slice(-2)}.${Math.floor((STATES.stageKumite.time % 1) * 10)}`
                if (STATES.stageKumite.time == 0) STATES.stageKumite.play = false
                if (STATES.stageKumite.time <= 16.10 && !STATES.shortBellPlayed) {
                    STATES.shortBellPlayed = true;
                    if (document.getElementById('short-bell')) document.getElementById('short-bell').play();
                }
                if (STATES.stageKumite.time > 16.10) STATES.shortBellPlayed = false;
    ...
    document.getElementById("match-content").innerHTML = `
        ...
        <div class="flex flex-col justify-center items-center bg-gray-300 p-4 rounded-lg">
            <div class="flex gap-4 mb-4">
                ...
                <div id="timer" class="text-6xl font-bold mx-4 text-center" style="width: 280px; flex-shrink: 0;">${Math.floor(STATES.stageKumite.time / 60)}:${`0${Math.floor(STATES.stageKumite.time%60)}`.slice(-2)}.${Math.floor((STATES.stageKumite.time % 1) * 10)}</div>
                <button class="py-2 px-4 bg-gray-600 text-white rounded-lg hover:bg-gray-500" onclick="handlerMatchControl(0, '+1s')">+1s</button>
                <button class="py-2 px-4 bg-gray-600 text-white rounded-lg hover:bg-gray-500" onclick="handlerMatchControl(0, '+30s')">+30s</button>
                <button class="py-2 px-4 bg-gray-600 text-white rounded-lg hover:bg-gray-500" onclick="handlerMatchControl(0, '+60s')">+60s</button>
            </div>
